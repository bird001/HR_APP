...
if ($_GET['check'] === "true") {
    ...
    $sql_insert_archive = "insert into ApplyLeaveArchive(id_val,Firstname,LastName,EmpID,EmpDept,EmpRole,ManagerEmail,LeaveType,StartDate,EndDate,NumDays,Reason)
                                select
                                id_val,Firstname,LastName,EmpID,EmpDept,EmpRole,ManagerEmail,LeaveType,StartDate,EndDate,NumDays,Reason
                                from ApplyLeave
                                WHERE id_val = '$id'";
    ...
    if ($role === 'Manager') {
        if ($column === 'Accept') {
            ...
            $sql_insert_hr = "insert into ApplyLeaveArchive(id_val,Firstname,LastName,EmpID,EmpDept,EmpRole,EmpEmail,ManagerEmail,HREmail,LeaveType,StartDate,EndDate,NumDays,Reason)
                                select
                                id_val,Firstname,LastName,EmpID,EmpDept,EmpRole,EmpEmail,ManagerEmail,HREmail,LeaveType,StartDate,EndDate,NumDays,Reason
                                from ApplyLeave
                                WHERE id_val = '$id'";
        ...
        if ($column === 'Reject') {
            $sql_getstuff = "SELECT * FROM ApplyLeave where ManagerEmail = '$email' and id_val ='$id'";
            $result = mysqli_query($conn, $sql_getstuff);
            $row = mysqli_fetch_array($result, MYSQLI_ASSOC);
            ...
            $sql_insert_hr = "insert into ApplyLeaveArchive(id_val,Firstname,LastName,EmpID,EmpDept,EmpRole,EmpEmail,ManagerEmail,HREmail,LeaveType,
            ...
            $sql_delete_hr = "delete from ApplyLeave where id_val = '$id'";
            mysqli_query($conn, $sql_delete_hr);
        }
    } else {
        //if the request was accepted
        if ($column === 'Accept') {
            if ($hremail === $email) {//accepted by HR
            ...
            if ($manemail === $email) {//accepted by manager
                $sql_getstuff = "SELECT * FROM ApplyLeave where ManagerEmail = '$email' and id_val ='$id'"; //get the specific row of data
                $result = mysqli_query($conn, $sql_getstuff);
                $row = mysqli_fetch_array($result, MYSQLI_ASSOC);
                ...
                $sql_insert_hr = "insert into ApplyLeaveHR(id_val,Firstname,LastName,EmpID,EmpDept,EmpRole,EmpEmail,ManagerEmail,HREmail,LeaveType,StartDate,EndDate,NumDays,Reason)
                                select
                                id_val,Firstname,LastName,EmpID,EmpDept,EmpRole,EmpEmail,ManagerEmail,HREmail,LeaveType,StartDate,EndDate,NumDays,Reason
                                from ApplyLeave
                                WHERE id_val = '$id'";
        ...
        //reject
        if ($column === 'Reject') {//if the leave application is rejected and it was done by an Hr
            if ($hremail === $email) {//done by HR
            ...
            if ($manemail === $email) { //done by manager
                $sql_getstuff = "SELECT * FROM ApplyLeave where ManagerEmail = '$email' and id_val ='$id'"; //get the specific row of data
                $result = mysqli_query($conn, $sql_getstuff);
                $row = mysqli_fetch_array($result, MYSQLI_ASSOC);
