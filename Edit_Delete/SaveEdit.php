...
if ($name == 'LoanPayments') {//for the LoanPayment table
    ...
    $balance = $row_results['BalanceOutstanding'];
    ...
    if ($loantype == 'Reducing') {
        ...
        $interestrepaid = round(($balance * ($loaninterest / 100)) / $loanterm, 2);
        $balancerepaid = round($newValue - $interestrepaid, 2);
    }

    if ($loantype == 'Straight') {

        $paymentTime = date("F d, Y");
        $interestrepaid = round(($amount * ($loaninterest / 100)) / $loanterm, 2);
        $balancerepaid = round($newValue - $interestrepaid, 2);
    }
    ...
    $sql = "UPDATE LoanApproved SET $column = :value, PaymentDate = '$paymentTime', InterestRepaid = '$interestrepaid', BalanceRepaid = '$balancerepaid',"
            . " Editable = '1' WHERE id_val = :id";
